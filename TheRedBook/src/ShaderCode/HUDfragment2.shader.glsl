...
bool drawColoredRays(Ray r) {
     ...
     int raysPerPixel = numPoints;//vertArr.length()/ 50;// vertArr.length();// int(ceil(vertArr.length() / 20)); // minimum teilen durch 4 oder mehr
    bool shadow = false;
    float imageWidth = widthHeight.x, imageHeight = widthHeight.y;
    int div = 1;
    float valX = 0.0f;//float(j / div);//for (int i = 0; i < (imageWidth / div); i++) {
    float valY = 0.0f; //float(l / div);     //for (int j = 0; j < (imageHeight / div); j++) {
    bool inLense = false;
...
{   
    ...
    int numCircles = 2, numPointsPerCircle = 5;
